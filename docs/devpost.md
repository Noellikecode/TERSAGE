...
| **Memory Bank** | **Adopted, for the half it fits.** Vertex AI Agent Engine Memory Bank holds the *prose* of every open question and answers "has anyone asked something like this" by meaning. The *record* — eliminations, evidence, examination counts, transitions, checkpoints — stays in Firestore, because a `Memory.fact` is capped at 2048 characters and a long-running thread's accumulated eliminations are not. We measured that rather than assumed it; four other assumptions about the service were also wrong and `scripts/verify_memory_bank.py` is what found them. Separately, the Vertex **Vector Search** adapter (narratives, a different port) is written and **off** — a running index endpoint is several hundred USD a month — so narrative recall is still the in-memory lexical index |
...
**We adopted the platform's Memory Bank and it corrected us five times.** The plan was to replace our own memory bank with Vertex AI Agent Engine Memory Bank outright. The SDK's message shapes made that look straightforward — client-supplied ids, a scope map, a filter — so we wrote the adapter against them. Then we ran it against the live service, and five things were not what the shapes implied. A `Memory.fact` is capped at **2048 characters**, which a question's prose fits and a long-running thread's accumulated eliminations do not — so the state machine stayed in Firestore and only the prose moved, which is the split we shipped. A duplicate create answers **`InvalidArgument`, not `AlreadyExists`**, and the same status code refuses an oversize fact, so the branch has to tell those two apart or a rejected write becomes a silent overwrite. Scope matching is **exact, not subset** — a memory carrying one extra scope key is invisible to a narrower query, returning empty rather than erroring, which would have made every district-wide recall silently find nothing forever. A retrieval hands back a **synthetic resource name with `display_name` blanked**, so the id we so carefully supplied does not come back and has to travel in the text. And **deleting a memory reserves its id permanently** — `get` says gone, `create` says taken — which for a system where `ABANDONED → RESOLVED` is the case the component exists for would have left every reopened thread unindexable, silently, because indexing failures are counted rather than raised.

Not one of those is discoverable from a unit test with a mocked client, because a mocked client asserts your own assumptions back at you. `scripts/verify_memory_bank.py` runs all thirteen checks against the real service and is the reason each of them is now a fact rather than a belief. **The one capability we refused is the one the product leads with:** `generate_memories` has a model decide what is worth remembering, and a model may not author anything in this system. Every memory we store is prose an agent already wrote into a question the deterministic path opened.
