...
| Nothing has ever run on Google Cloud | Open — needs you | 30% of the score. No credentials on the build machine. Everything Vertex-facing is written and unrun. |
| Model ids unverified | Open — needs you | `gemini-3.5-flash` and `gemma-3-4b-it` have never been resolved against a real endpoint. If either is wrong every live call fails. |
