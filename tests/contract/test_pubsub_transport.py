...
One broker satisfies it: ``PUBSUB_TEST_PROJECT``, real Pub/Sub reached through
Application Default Credentials. There is no emulator path, deliberately --
message ordering is the property under test here, and it is exactly the kind of
thing an emulator approximates rather than reproduces.

Skips loudly unless it is configured. A skipped transport has proved nothing,
and CI fails the job if this suite reports a skip.
...
#: Only read once a broker is confirmed configured, so the fallback is never
#: reached in a run that publishes anything.
PROJECT = REAL_PROJECT or ""
...
def _require_broker() -> None:
    if not REAL_PROJECT:
        pytest.skip(
            f"{PROJECT_ENV} is not set; run "
            "`make test-cloud GCP_TEST_PROJECT_ID=your-test-project` after "
